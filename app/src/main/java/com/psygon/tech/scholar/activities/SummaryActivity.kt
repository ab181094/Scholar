...
class SummaryActivity : ParentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            ...
            val chapterString = bundle.getString(EXAM_KEY)

            val collectionType = object : TypeToken<MutableList<ExamModel>>() {}.type
            examList = Gson().fromJson(chapterString, collectionType)
        }
        ...
        storeSummary(auth, currentChapter.key, currentChapter.name, CHAPTER, correctCount, examList.size)
        ...
        fab.setOnClickListener {
            ...
            shareString += " $CORRECTLY_OUT_OF ${examList.size} $IN ${currentChapter.name}" +
                    ", ${currentTopic.name}, ${currentSubject.name}"
